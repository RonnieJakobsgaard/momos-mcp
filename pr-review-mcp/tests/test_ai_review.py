...
def test_ai_pre_review_injects_valid_comments():
    ...
    with patch("janus_mcp.ai._HAS_ANTHROPIC", True), \
         patch("janus_mcp.ai.os.environ.get", return_value="fake-key"), \
         patch("janus_mcp.ai._anthropic.Anthropic", return_value=_make_client(comment)):
...
def test_ai_pre_review_skips_malformed_lines():
    ...
    with patch("janus_mcp.ai._HAS_ANTHROPIC", True), \
         patch("janus_mcp.ai.os.environ.get", return_value="fake-key"), \
         patch("janus_mcp.ai._anthropic.Anthropic", return_value=_make_client(output)):
        server._run_ai_pre_review("diff text")
...
def test_ai_pre_review_no_op_when_anthropic_missing():
    with patch("janus_mcp.ai._HAS_ANTHROPIC", False):
        server._run_ai_pre_review("diff text")
...
def test_ai_pre_review_no_op_when_api_key_missing():
    with patch("janus_mcp.ai._HAS_ANTHROPIC", True), \
         patch("janus_mcp.ai.os.environ.get", return_value=None):
        server._run_ai_pre_review("diff text")
...
def test_ai_pre_review_no_op_on_api_exception():
    ...
    with patch("janus_mcp.ai._HAS_ANTHROPIC", True), \
         patch("janus_mcp.ai.os.environ.get", return_value="fake-key"), \
         patch("janus_mcp.ai._anthropic.Anthropic", return_value=mock_client):
        server._run_ai_pre_review("diff text")
...
def test_suggest_commit_message_returns_string():
    with patch("janus_mcp.ai._HAS_ANTHROPIC", True), \
         patch("janus_mcp.ai.os.environ.get", return_value="fake-key"), \
         patch("janus_mcp.ai._anthropic.Anthropic", return_value=_make_client("feat: add null check")):
...
def test_suggest_commit_message_returns_none_on_empty_response():
    with patch("janus_mcp.ai._HAS_ANTHROPIC", True), \
         patch("janus_mcp.ai.os.environ.get", return_value="fake-key"), \
         patch("janus_mcp.ai._anthropic.Anthropic", return_value=_make_client("")):
        assert server._suggest_commit_message("diff text") is None
